...
Definitions 1–7, Theorems 1–4 and Lemmas 1–4 appear in both, at the same
numbering, with `Ax-1`/`Ax-2`/`Ax-3`, Split Representations, Trivial-R,
Threshold/Anti-Threshold, `Δ^R₀`/`Π^R₁`, `MinG`, `B`-boundedness, the
Conventional Encoding Method and `Diag(Ax-3)` all already in place.
...
So unlike `Willard2000-TAB` (whose journal version is a strict superset) and
`Willard2020-LFCS` (whose journal version **drops** material), this pair is a
clean expansion: the conference form is a proper part of the journal form.
