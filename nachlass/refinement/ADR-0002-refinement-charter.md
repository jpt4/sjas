...
| **R4** | The computational analogue: test the type-theoretic reading against Brown–Palsberg. **Carries obligation RO1**: affineness alone does not evade G2 — `□`-contraction can hold in affine PA (Beklemishev–Shamkanov §3). **And, from R6 (`ADR-0004` Consequences, recorded here 2026-09-04): a precise transfer target** — a coded modal proof doctrine in which the boxed diagonal `copy_A : □A → □A ⊗ □A` and the uniform proof-composition map are exhibited *separately*, so a candidate type theory can be tested against each independently rather than against "self-reference" as a whole. **Working notes 2026-09-07: [`R4-notes-native-model.md`](R4-notes-native-model.md)** — the term-grammar guard rail, Remark 6's admission rule for new function symbols, the withdrawal of the decidable-index-language framing, what `Tab` costs operationally, and the `Z` dial as the subset that buys modus ponens back. **Carries obligation RO3** (opened 2026-09-07, ibid. §4): has any DML-style or refinement-type system been given a *self-referential* index theory? Nothing verified, no witness held | pending |
