...
**Status.** R1 done and reviewed; R2 done; R3 partly done; **R6 complete and completion-reviewed 2026-09-04**; R4 in progress (draft, proofs and implementation, 2026-09-25/26); R5 pending.
...
| **R4** | The computational analogue: test the type-theoretic reading against Brown–Palsberg. **Carries obligation RO1**: affineness alone does not evade G2 — `□`-contraction can hold in affine PA (Beklemishev–Shamkanov §3). **And, from R6 (`ADR-0004` Consequences, recorded here 2026-09-04): a precise transfer target** — a coded modal proof doctrine in which the boxed diagonal `copy_A : □A → □A ⊗ □A` and the uniform proof-composition map are exhibited *separately*, so a candidate type theory can be tested against each independently rather than against "self-reference" as a whole. **Working notes 2026-09-07: [`R4-notes-native-model.md`](R4-notes-native-model.md)** — the term-grammar guard rail, Remark 6's admission rule for new function symbols, the withdrawal of the decidable-index-language framing, what `Tab` costs operationally, and the `Z` dial as the subset that buys modus ponens back. **Carries obligation RO3** (opened 2026-09-07, ibid. §4): has any DML-style or refinement-type system been given a *self-referential* index theory? Nothing verified, no witness held. **Draft calculus 2026-09-25: [`R4-certificate-calculus.md`](R4-certificate-calculus.md)** — λᶜᵉʳᵗ, an affine certificate sort over Hofmann's `◇`, with a consistency theorem conditional on four open lemmas. **Proofs 2026-09-25/26: [`R4-metatheory.md`](R4-metatheory.md)** — consistency, the pair form `H₁`, size soundness and termination for a core calculus, by a budget-stratified model; G2 for codes by an outlined reduction to PA; independently reviewed, two rounds. **Implementation 2026-09-26: [`ADR-0005-lcert-implementation.md`](ADR-0005-lcert-implementation.md)** — the core calculus as a language, with an Ansatz-verified kernel | in progress |
| **R5** | Secondary-literature pass: Artemov, Pudlák, Visser, Adamowicz, Kolodziejczyk, Salehi. **Carries obligation RO2** (opened 2026-09-07, [`RO2-contraction-growth-rate.md`](RO2-contraction-growth-rate.md)): Beklemishev–Shamkanov state the contraction prerequisite structurally and give no size claim, while R6 §7.4's obstruction is quantitative. Is there a derivation-size theorem behind their Proposition 3.8, and does a resource-indexed `□` satisfy the derivability conditions up to its index and fail them uniformly? **Recorded as worth pursuing on its own account, as a contribution to their line rather than an instrument of this one.** Needs acquisitions: no light-linear-logic witness is held | pending |
...
- **B6** *(added 2026-09-02; restated twice)* Every mathematical quotation image-verified **and transcribed verbatim**, and every corpus claim status-carrying, per [`VERIFICATION.md`](VERIFICATION.md), with `../codification/audit.sh` green. *(**This criterion has now been declared met and then falsified four times** — corrections #23/#32, when two quotations tagged "image-verified" proved non-verbatim; #62–#66, when a sweep found the status-carrying half broken in ten places including `Willard2001` Thm A.1 and `Willard2011` Lemma 4.6; **#120**, when `Willard2001` §1's futility verdict proved to have been cut mid-sentence in a way that inverted it; and the **thirtieth pass**, which found two `Willard1993-TR` anchors that had never been read from the page at all — one confirmed only against an extraction record, which rule 1 says is not a source. That last is a failure of the *image-verified* half, not the verbatim half, and it is the kind this criterion had not previously caught: a quotation can be faithful to a record that is itself derived. The count in this entry was itself stale at "twice" until 2026-09-05 — the criterion written to resist standing assertions went stale on its own record of them. Neither half is machine-checkable: R-B tests register hygiene, not fidelity, and R-E is informational because no positional heuristic captures "at the point of use". **Treat any future assertion that B6 is met as a claim requiring a fresh sweep, not as a standing fact.** The first pass asserted this while two quotations tagged "(image-verified)" were not verbatim — corrections #23 and #32. Note what green does **not** mean: `../codification/audit.sh` cannot check quotation fidelity. R-B tests only whether a `txt`-tagged register row carries hazard symbols; nothing diffs a quotation against its source. Verbatim transcription is a **human obligation** that the audit supports and does not discharge.)*
