...
| **Theorem 1 (Cantor's Theorem)**: "If `Y` is a set and there exists a function `α : Y ⟶ Y` without a fixed point (for all `y ∈ Y`, `α(y) ≠ y`), then for all sets `T` and for all functions `f : T × T ⟶ Y` there exists a function `g : T ⟶ Y` that is not representable by `f` i.e. such that for all `t ∈ T` `g(−) ≠ f(−,t)`." | `Yanofsky2003` | printed/pdf p. 5 | **img (2026-09-06)** — the negative form `R6-g2-via-lawvere.md` §5.3 instantiates |
| **Theorem 2**: "Let `Y` be a set, `α : Y ⟶ Y` a function without a fixed point, `T` and `S` sets and `β : T ⟶ S` a function that is onto (i.e., has a right inverse `β̄ : S ⟶ T`,) then for all functions `f : T × S ⟶ Y` the function `g_β : T ⟶ Y` constructed as follows … is not representable by `f`." | `Yanofsky2003` | printed/pdf p. 6 | **img (2026-09-06)** — the `⟨Id,β⟩` generalization that repairs the source's own §5 instantiation (Departure Y1). The printed proof writes `β(β̄(t₀))` for `β(β̄(s₀))`; typographical, nothing rests on it |
| **Theorem 3 (Diagonal Theorem)**: "If `Y` is a set and there exists a set `T` and a function `f : T × T ⟶ Y` such that all functions `g : T ⟶ Y` are representable by `f` (there exists a `t ∈ T` such that `g(−) = f(−,t)`,) then all functions `α : Y ⟶ Y` have a fixed point." | `Yanofsky2003` | printed/pdf p. 14 | **img (2026-09-06)** — the positive form; `R6-g2-via-lawvere.md` §5.2 uses its `β`-variant |
| **Remark 5**: "It is important to note that the theorem uses a stronger hypothesis than the proof actually uses. The theorem asks that **all** `g : T ⟶ Y` be representable, however the proof only uses the fact that any `g` constructed in such a manner is representable. In the future, we shall use this fact and only require that constructed `g` be representable." | `Yanofsky2003` | printed/pdf p. 15 | **img (2026-09-06)** — the hinge of `R6-g2-via-lawvere.md` Proposition 7.1, which weakens it one step further |
| The §5 instantiation, quoted because it is the one this stage corrects: `f : Lind¹ × Lind¹ ⟶ Lind⁰` with "`f(B(x), H(y)) = H(⌜B(x)⌝)`", and "`Φ_E : Lind⁰ ⟶ Lind⁰` be defined as `P ↦ Φ_E(P) = E(⌜P⌝)`" | `Yanofsky2003` | printed/pdf pp. 15–16 | **img (2026-09-06)** — neither is well defined on Lindenbaum classes; Departure Y1, `R6-g2-via-lawvere.md` §2.3 |
| Gödel's First application: "`E(x) ≡ (∀y)¬Prov(y,x)`. A fixed point for this `E(x)` in a consistent and ω-consistent theory is a sentence that is equivalent to its own statement of unprovability." | `Yanofsky2003` | printed/pdf p. 16 | **img (2026-09-06)** — the same endofunction `R6-g2-via-lawvere.md` §5.2 uses for G2, and the ω-consistency the G2 route does not need |
| The future-work bullet this stage answers: "Gödel's second incompleteness theorem about the unprovability within arithmetic of the consistency of arithmetic. This theorem is a simple consequence of the first incompleteness theorem. However Kreisal has a direct model theoretic proofs that uses a diagonal method (see, e.g., page 860 of Smoryński's article in [1].) This proof seems amenable to our scheme." | `Yanofsky2003` | printed/pdf p. 22 | **img (2026-09-06)** — verbatim, misspelling and plural in the source; under the heading "The following ideas are a little more 'spacey.'" |
| The closing bullet: "We worked out Gödel's first incompleteness theorem which showed that … arithmetic can not completely talk about its own provability. What about Gödel's completeness theorem? Certain weak systems can completely talk about their own provability. Can this be stated as some type of fixed point theorem?" | `Yanofsky2003` | printed/pdf p. 22 | **img (2026-09-06)** — quoted with the declaration that its context is the completeness theorem; the SJAS reading is the Refinement's, not the source's |
| Appendix A notation (iii): "`SUBST_i(x,y)` denotes Godel's classic substitution **relation** which yields TRUE when `x` is a Godel number of a formula that contains a free variable `v̂[i]`, `y` is the Godel number of a formula generated by replacing each appearance of "`v̂[i]`" with the Godel number `x` …" | `Willard1993-TR` | printed p. 37 (pdf p. 39) | **img (2026-09-06)** — substitution is a *relation* from its first definition, never a function symbol; the premise of `R6-g2-via-lawvere.md` §5.2's relational route |
| The diagonal number is named and unique: "(These equations enable us to formalize `Prf_{IS(A)}(x,y)` because **the only integer `z` satisfying `SUBST_i(J,z)`** in equations (A.2) & (A.3) is (A.2)'s Godel number.)" and Clarifying Comment 1's "if `K` denotes the **unique** Godel number satisfying `SUBST_i(J,K)`" | `Willard1993-TR` | printed p. 38 (pdf p. 40) | **img (2026-09-06)** — `IS(A)` constructs its diagonal number and its whole Group-3 encoding turns on `K`'s uniqueness; refutes the reading that the construction of the number is what `IS(A)` gives up (`R6-g2-via-lawvere.md` §7.1) |
| The size argument is the totality argument: "If `j` is a very large number, then its repeated appearance in a proof could cause the proof to grow to an unacceptably large size. Therefore, our notation will employ two special symbols to reduce the bit length of a proof", and **Lemma 5.2**'s parenthetical "(The reason for the new "`v#`" notation is that the two analogous mappings *onto* `⌜Φ(ĉ[j])⌝` *are unknown by* `IS(PA+)` *to be total functions* !)" | `Willard1993-TR` | printed p. 15 (pdf p. 17) | **img (2026-09-06)** — Lemma 5.2 is `full`; the same page fixes integers at `⌊log₃₂ i⌋ + 3` bytes "so as to achieve the ideal 'linear' bit compression". Cited at `R6-g2-via-lawvere.md` §7.1 for why the corpus's bit-accounting is an existence argument, not an optimisation |
| The function symbol *is* the totality assumption: "since our U-Grounding language has no function symbol for multiplication, our U-Grounding based axiom systems `α` **do not implicitly assume** that multiplication is a total function. However, they do certainly retain an ability to encode the graph `M(x,y,z)` of multiplication as a `Δ*₀` formula". Same page, on why `Π*_n`/`Σ*_n` are needed: conventional `Π_n`/`Σ_m` notation "(with its multiplication function symbol) is suitable only for axiom systems which recognize multiplication as a total function", since the bound `t` of a bounded quantifier must be a **term** | `Willard2005` | printed/pdf p. 5 | **img (2026-09-06)** — the object-language form of `R6-g2-via-lawvere.md` §5.1's H-C-fn / H-C-pt split, cited at §5.2 |
| **Equation (4)**, multiplication's graph over the Division grounding function: "`[ (x = 0 ∨ y = 0) ⇒ z = 0 ] ∧ [ (x ≠ 0 ∧ y ≠ 0) ⇒ ( z/x = y ∧ (z−1)/x < y ) ]`", glossed as giving "an understanding of multiplication's main generic properties (except of course for the *deliberately omitted* totality condition of "`∀x ∀y ∃z M(x,y,z)`".)" | `Willard2005` | printed/pdf p. 5 | **img (2026-09-06)** — read from the page because `pdftotext` renders `≠` as `6=` and drops the division bars (hazard H2). "Deliberately omitted" is Willard's phrase, and it is the anchor for `R6-g2-via-lawvere.md` §5.2's object-language parallel |
| **Lemma 5.1**, the definition of majorization: "Say a formula `Φ(x₁x₂…x_L)` is **majorized with exponent λ** iff each universally and existentially quantified variable `v` in `Φ` is required to be less than `x_i^λ` for some `i<L`. Then for each majorized formula `Ψ(x₁x₂…x_L)` there exists an "essentially equivalent" `Δ₀` formula `Ψ*(x₁x₂…x_L)` …" — with the mechanism in the Informal Proof Sketch: "replace each `∃v < x^λ` with the sequence `∃v₁<x ∃v₂<x ⋯ ∃v_{2λ}<x` … when the string `v` is so divided into `2λ` corresponding smaller strings" | `Willard1993-TR` | printed p. 14 (pdf p. 16) | **img (2026-09-06)** — Lemma 5.1 is `sketch` ("Informal Proof Sketch" is the source's own heading). A big object is simulated by a **fixed-length tuple** of in-domain objects; `2λ` is fixed by the formula, which is why a product of two *variables* is not majorized |
| The framing sentence for the whole device: "One difficulty will be that multiplication (and hence also `x^λ` (for *fixed exponent* `λ`)) are not functions under IS. Lemma 5.1 illustrates how we can overcome this difficulty by treating `x^λ` *almost* as if it was a function!!!"; and its consequence, "Lemma 5.1 assures that the absence of a formal multiplication function in `IS(A)` does not raise any new complications, with regards to linearly equivalent encodings" | `Willard1993-TR` | printed p. 14 (pdf p. 16) | **img (2026-09-06)** — "*almost* as if it was a function" is Willard's own name for the functional/relational split that `R6-g2-via-lawvere.md` §5.1 calls H-C-fn vs H-C-pt |
| **Lemma 7.1's proof**, the product that governs proof-composition: the `PA+` `Π₁` theorem "`∀t ∀p ∀y` if `t` is a terse proof of `¬Φ` and `p` is a semantic tableaux proof of `Φ` and `y = 64 · ⌜¬⊥⌝ · t · p` then `∃q<y` satisfying `Prf_{IS(A)}(⊥,q)`", usable inside `IS(PA+)` only because "While `IS(PA+)` can not recognize multiplication as a function, it certainly can verify the scalar product `kp = p+p+p+…+p` is a function", giving Eq. (7.1) `∀p ∃y { y = kp = 64·⌜¬⊥⌝·g·p }` | `Willard1993-TR` | printed p. 24 (pdf p. 26) | **img (2026-09-07)** — the mechanism of the printed-p. 12 HBL (2) claim: the glued proof's code is bounded by the **product of the two proof codes**, and one factor must be a constant. `pdftotext` renders `⌜¬⊥⌝` as `[41]`/`[AL]` — hazard H2. Cited at `R6-g2-via-lawvere.md` §7.4 |
| **Remark 2, the Meta-Logic convention**: "our proofs shall *apply a cut rule at the meta-theoretical level*. … suppose it is known that a *cut-free* `IS(PA+)` system can prove the two statements `α` and `α ⊃ β`. Then by the Cut Elimination Theorem …, `IS(PA+)` will also support a cut-free proof of `β`. … we will *often omit constructing formally the cut-free proofs* of such theorems `β`: Instead, *their theoretical existence* will be established implicitly via the Cut Elimination Theorem" — and it "will be used frequently in the remainder of this paper" | `Willard1993-TR` | printed p. 24 (pdf p. 26) | **img (2026-09-07)** — the passage licensing `lawvere-sjas.md` §4.2's claim that composition is supplied metatheoretically by cut elimination, which that section asserted without a citation. Codification obligation **O19**, open under **G40** |
| **Lemma 5.3's proof**, the same product at another site: "`T(p,k) ≤ L_k·p` (where `L_k` is a constant depending on `k`)" and "`IS(PA+)` will recognize multiplication by any *scalar constant* as a total function because `L_k p = p+p+p+…+p`" | `Willard1993-TR` | printed p. 18 (pdf p. 20) | **img (2026-09-07)** — third of the four sites tabulated at `R6-g2-via-lawvere.md` §7.4 |
| **The Proposition 1 mechanism**: "If `IS(A)` had included multiplication as a function, it would be possible for an inconsistency proof `p` to construct within itself a representation of the necessary `p* ≥ p`. … However a crucial difference between Propositions 1 and 6 is that the Group-1 Axioms of the former contain **only Addition as a growth function**. … In particular, Addition grows so slowly that `log(x + y) ≤ MAX(log x, log y) + 1`. This implies essentially that it is impossible in `log₂p − 1` bits to encode a number as large as `p`." | `Willard1993-TR` | printed p. 13 (pdf p. 15) | **img (2026-09-07)** — the growth inequality on which the consistency of `IS(A)` turns, and the reason the same product is fatal everywhere it appears |
| **Definition D.1(iv)**, the encoding promoted to a condition: "`ξ*`'s Gödelized method `g` for encoding a semantic tableaux proof can be **essentially any natural method** that satisfies the minor stipulation that at least `5J` bits are required to encode a semantic tableaux proof that has `J` function symbols" | `Willard2011` | printed p. 38 (pdf p. 39) | **img (2026-09-07)** — with footnote 23 (already registered, printed p. 39) this makes the density a **lower bound derived by counting**, so the composition product survives every admissible re-encoding. Cited at `R6-g2-via-lawvere.md` §7.4 |
| **Footnote 22**, why the bound makes the difference: of Eq. (46) `∀x ∀y ∃z ≤ x+y : { z = x+y }` — "The virtue of (46)'s format is that it is a `Π*₁` styled statement, unlike (4)'s `Π*₂` styled format." And on the same page: "(Addition can be treated trivially because the U-grounding language possesses an addition function symbol.)" | `Willard2011` | printed p. 38 (pdf p. 39) | **img (2026-09-07)** — the complexity of a totality axiom is set by whether a **term** can bound its existential quantifier, which is why addition-totality is `Π*₁` and multiplication-totality and uniform proof-composition are `Π*₂`. Cited at `R6-g2-via-lawvere.md` §7.4 |
| **Remark D.5**: "the declaration that multiplication is a total function is *the trigger-point* causing the semantic tableaux version of the Second Incompleteness Theorem to become active. This **threshold effect** is significant…"; footnote 31: "**We formally proved** in [60, 62, 67, 68] that multiplication's totality property causes the semantic tableaux version of the Second Incompleteness Theorem to become active" | `Willard2011` | printed p. 45 (pdf p. 46) | **img (2026-09-07)** — the "adding the totality is fatal" direction is claimed as formally proved; the "it is not derivable" direction is not. Bears on `R6-g2-via-lawvere.md` §7.4 and §8 |
| **The "cut-free" property, defined structurally**: "Every semantic tableaux or resolution proof [Fi90, Sm68] of `⊥` from an axiom system `A` must have every sentence of the proof tree (except possibly its root) constitute a **subcomponent** of some axiom of `A`. This characteristic of semantic tableaux and resolution proofs will be called their "cut-free" property. It is closely analogous to Gentzen's notion [Ge69,Ta87] of a cut-free sequent calculus proof. … our theorem technically applies to semantic tableaux deduction, although it trivially generalizes to resolution, as well as **any formal "cut-free" deduction method**." | `Willard1993-TR` | printed p. 20 (pdf p. 22) | **img (2026-09-07)** — the subcomponent relation is defined by five clauses on printed pp. 19–20, clause (iii) being that `X(u)` and `X(c)` are subcomponents of `∀a X(a)` and `∃a X(a)`. Cited at `R6-g2-via-lawvere.md` §7.4 |
| **Lemma 6.2**: "If the axiom system `A` is nice then a semantic tableaux proof `p` of `⊥` from `IS(A)` is impossible without `p` formally constructing a parameter or constant element `p*`, such that one node of `p`'s proof tree is the sentence `¬Prf_{IS(A)}(⊥, p*)`", with **Lemma 6.1**: "The sentence (6.1) is not a subcomponent of any Group 1 or 2 axiom of `IS(A)`", where (6.1) is `¬∀y ¬Prf_{IS(A)}(⊥,y)` | `Willard1993-TR` | printed p. 20 (pdf p. 22) | **img (2026-09-07)** — Lemma 6.1 is **`sketch`** (the source labels it "Proof Sketch"), Lemma 6.2 is **`full`**. Together they are why Group-3's safety is a property of the *sentence plus the deduction method*, not of the arithmetic alone: the axiom can only be contradicted through an instantiated subcomponent, so the witness must be named in the tree |
| **Definition 3.4**, the derivability conditions: "`□ : L_S → L_S` satisfies Löb's derivability conditions for `S` if **L1.** `□(φ → ψ) ⊢ □φ → □ψ`; **L2.** `□φ ⊢ □□φ`; **L3.** `⊢ φ` implies `⊢ □φ`"; with **Lemma 3.5** and **Remark 3.6**: "Notice that the last rule is formulated slightly differently from the more standard rule for modal logic K4: `Γ, □Γ ⊢ φ / □Γ ⊢ □φ`. **The latter has a form of built-in contraction that we are not assuming here.**" | `B–S 2016` | printed p. 7 | **img (2026-09-07)** — discharges `RO1-affine-separation.md`'s outstanding verification; every box it reconstructed from context checks out, and L1 is indeed curried and stated as a sequent |
| **Proposition 3.8** and its contraction step: "Suppose `S` satisfies contraction, `□` satisfies Löb's conditions for `S` and `⊠φ := □(φ → ⊥)`. Then `(L_S, ≤_S, □, ⊠, ⊤, ⊥)` is an APS", proved via "The rules of transitivity **and contraction** imply that, if `Γ ⊢ □φ` and `Γ ⊢ □¬φ`, then `Γ ⊢ □¬⊤`"; and **Definition 3.7**'s note that "for Girard's linear logic this condition is not met… a certain amount of contraction is essential for the proof of G2"; and **Remark 3.9**'s `□`-contraction, `□`-weakening, C3′, C5′ | `B–S 2016` | printed p. 8 | **img (2026-09-07)** — confirms RO1 §1's reading that contraction is consumed at the C3 step, on the context `Γ` |
| **Theorem 1** and the collapse argument: "Suppose an APS `S` has a Gödelian fixed point. (i) If `S` is consistent, then `⊠⊤` is irrefutable in `S`. (ii) `⊠⊠⊤ ⩽_S ⊠⊤`, that is, Statement (i) is formalizable in `S`", proof of (i): "Assume `⊠⊤ ⩽ ⊥`. … Therefore, by C2, `⊤ ⩽ ⊠⊥ ⩽ ⊥`"; with **Remark 2.4**: "we take `⊠⊤` as our default expression of inconsistency" | `B–S 2016` | printed p. 5 | **img (2026-09-07)** — for an **APS** with a Gödelian fixed point, adjoining consistency *is* collapse; `S` escapes only by not being an APS. Cited at `RO2-contraction-growth-rate.md` §4 |
| **The affine-PA remark, at the end of §3**: "The property of `□`-contraction actually holds for some meaningful arithmetical systems lacking general contraction rule, for example, for a version of **Peano arithmetic based on affine predicate logic** considered by the second author of this paper (as yet, unpublished)"; and §4's definition of `S` as "a version of modal logic K4 based on the multiplicative `{→, ⊗, ⊥}` fragment of a classical logic without contraction" with a built-in fixed point operator `fp x.A(x) =_S A(fp x.A(x))` | `B–S 2016` | printed p. 9 | **img (2026-09-07)** — **confirms the `§3` citation** in `refined-sjas.md` and `RO1-affine-separation.md` §5, which a 2026-09-07 check had suspected of being a mis-citation for §6. It is not; both sentences exist, in §3 and §6 |
| **Theorem 5**, cut-admissibility with a size bound: "The cut rule … is admissible for `S`. Moreover, if `π₁` and `π₂` are proofs of the premises of (cut), then the conclusion of (cut) has a proof with **the size being less than `‖π₁‖ + ‖π₂‖`**", where `‖π‖` is "the number of nodes in `π`" | `B–S 2016` | printed p. 11 | **img (2026-09-07)** — **refutes** `RO2-contraction-growth-rate.md`'s opening premise that B–S "carry no size claim". This is a strict linear-sum bound, the same currency as Willard's Linear-Sum Effect |
| **§6's closing conjecture**: "We remark that the system `S` does not provide a counterexample to the non-formalized version of G2, since `⇒ ¬□⊥` is not provable. **We believe that such a counterexample can be constructed by extending the language of `S` by an operator similar to `!` from linear logic and adding to `S` a fixed point of the form `a = ◇!a`.** However, a confirmation of this hypothesis is left for future work"; and "we are still missing convincing examples of mathematical theories based on weak logics for which G2 would fail" | `B–S 2016` | printed p. 14 | **img (2026-09-07)** — the authors state this programme's primary objective themselves. `RO2-contraction-growth-rate.md` §4 |
| **The Linear-Sum Effect**: "modus ponens assures that a proof of a theorem `ψ` from an axiom system `α` has a length **no greater than the sum** of the proof-lengths needed to derive `φ` and `φ → ψ` from `α`. This **"Linear-Sum Effect"** does not apply, actually, also to *Tab*-deduction because it owns no analog of a modus ponens rule"; and `Xtab`'s LEM node "will allow an *Xtab* proof for `ψ` to have a length proportional to the sum of the proof lengths for `φ` and `φ → ψ` (i.e. it can roughly simulate the actions of modus ponens)" — "similar to the **proof compressions** resulting from a modus ponens operation" | `Willard2020` | printed p. 279 (pdf p. 12) | **img (2026-09-07)** — corrects `R6-g2-via-lawvere.md` §7.4, which had read the additive size increase as harmless and located the whole obstruction in the code arithmetic. The danger is composed proofs being too **short** |
| The resource type: "The crucial innovation is a special resource type ◇ which has no constructors and hence no closed terms"; successor needs a `◇` argument, and "such an element cannot be generated 'out of nothing'" | `Hofmann 2003` | printed p. 59 (pdf p. 3) | **img (2026-09-25)** — `R4-certificate-calculus.md` §2.3 |
| Operator typing, rule T-Op, `∅ ⊢ eᵢ : Aᵢ`; "Notice that an operator is applicable to closed terms only" | `Hofmann 2003` | printed p. 61 (pdf p. 5) | **img (2026-09-25)** |
| Why step functions must not grow: "If h₀ or h₁ increase the size by a constant (as would be the case if they were allowed to contain variables) then itᴺ(g,h₀,h₁) would multiply the size by that constant, thus violating the intended invariant" | `Hofmann 2003` | printed p. 63 (pdf p. 7) | **img (2026-09-25)** |
| Trees: `node : ◇ ⊸ A ⊸ T(A) ⊸ T(A) ⊸ T(A)`; "The size of such a tree is given by the number of its nodes plus the sizes (w.r.t. s_A) of all its labels" | `Hofmann 2003` | printed pp. 65–66 (pdf pp. 9–10) | **img (2026-09-25)** |
| Restricted duplication `dup_{D,P} : (D ⊸ P) ⊸ D ⊸ (P ⊗ D)` for a datatype `D` and passive `P` (§6, Theorem 6.1); and "If we would not require that P be passive then we could define a diagonal map δ : D ⊸ D ⊗ D … We have already seen in the Introduction that this leads to exponential growth" | `Hofmann 2003` | printed pp. 78–79 (pdf pp. 22–23) | **img (2026-09-25)** |
| §9's title, "Borrowing does not work", on a constant `borrow_X : (◇ ⊸ (◇ ⊗ X)) ⊸ X`; and, same page, "In fact under the reading of ◇ as a certain amount of memory space proposed in [9] such borrowing would correspond to the region-based memory management introduced in [14]" | `Hofmann 2003` | printed p. 82 (pdf p. 26) | **img (2026-09-25)** — the memory reading is quoted in `R4-certificate-calculus.md` §2.7 |
| Trees with shared children: for `T′(A)`, whose `node : ◇ ⊸ A ⊗ (T′(A) × T′(A)) ⊸ T(A)` takes a cartesian pair, "Notice that if e ⊩_{T′(A)} t then l_e is an upper bound on the depth of t rather than its number of nodes. Accordingly, it is possible to define a function f : N ⊸ T′(N) which gives the full binary tree of depth [the binary length of n]"; and reference [9], "A type system for bounded space and functional in-place update" | `Hofmann 2003` | printed p. 84 (pdf p. 28) | **img (2026-09-25)** — `R4-certificate-calculus.md` §8.2–8.3: a certificate type must take its children multiplicatively |
| McBride's "insight is to use the 0 of the semiring to represent information that is erased at runtime, but is still available for use in types (i.e., extensionally)"; and the contribution "to fix a bug in the original system that caused substitution to be inadmissible" | `Atkey 2018` | p. 1 | **img (2026-09-25)** — `R4-certificate-calculus.md` §§2.1, 3 |
...
| **Why `2J` logical symbols "thus employ at least `5J` bits"** (`Willard2011` footnote 23, printed p. 39) | `R6-g2-via-lawvere.md` §7.4 | The step counts a **sequence**, not a count: `2J` symbols over an alphabet of size `k` admit `k^{2J}` strings and need `2J·log₂k` bits. `Willard1993-TR` printed p. 15 fixes six logical connectives (`∧ ∨ ¬ ⊃ ∀ ∃`), and `2J·log₂6 = 2J(2.585) ≈ 5.17J ≥ 5J`. The constant is `log₂32 = 5`, matching the `32^J` form. **Willard states the conclusion without the computation; this derivation is the Refinement's.** Note it does *not* justify D.1(iv) as a theorem — counting bounds the aggregate (pigeonhole over all proofs with `J` function symbols), while D.1(iv) demands the bound of every individual proof. Willard calls it a "minor stipulation", which is the correct status |
| **`64·⌜¬⊥⌝·t·p` covers the glued proof's code** (`Willard1993-TR` Lemma 7.1, printed p. 24) | `R6-g2-via-lawvere.md` §7.4 | Concatenating byte-strings multiplies: `t` followed by `p` has value `N_t·32^{|p|} + N_p`. With no leading zero byte, `N_p ≥ 32^{|p|−1}`, so `32^{|p|} ≤ 32·N_p` and the concatenation is `≤ 32·N_t·N_p + N_p ≤ 33·N_t·N_p`. A constant factor of `64·⌜¬⊥⌝` therefore covers the concatenation plus the root sentence `{¬⊥}` and a separator byte. **The missing operation is left shift by a variable amount, `x·32^y`**; `G₀`'s `Shift(x,y) = x/2^y` (printed p. 3) is the shrinking direction and is total |
| **No U-Grounding term exceeds a constant multiple of its largest argument** | `R6-g2-via-lawvere.md` §7.4; the 2026-09-07 log entry on shift-and-add | Induction on term structure, over Willard's own classification (`../codification/registry/notation.md`: six **non-growth** Grounding functions — Subtraction, Division, Root, Maximum, Logarithm, Count — plus the two **growth** functions Addition and Double). Base: a variable or constant is `≤ max(x⃗)` or `≤ c`. Non-growth step: each of the six maps arguments bounded by `M` to a value bounded by `M` (`x−y ≤ x`, `⌊x/y⌋ ≤ x`, `⌈x^{1/y}⌉ ≤ x` for `y ≥ 1`, `max ≤ M`, `Log x ≤ x`, `Count x ≤ x`). Growth step: `x+y ≤ 2·max(x,y)` and `2x`. So a term `t` containing `a_t` occurrences of Addition/Double satisfies `t(x⃗) ≤ 2^{a_t}·max(x⃗)`, with `a_t` fixed **by the term**. Hence no term majorizes `x·y`, nor the composed code `≈ N_t·N_p`, for variable arguments. **The Refinement's derivation, resting on Willard's classification, not a corpus theorem.** Edge conventions (`x/0 = 0`, truncated subtraction, `Root` at `y = 0`) are as the corpus fixes them and do not affect the bound |
